<script setup>
  ...
  const currentBuyer = ref("JAKO");
  const authStore = inject('userAuthStore')
  const currentReviewer = computed(() => authStore.user || '')
  const menuName = ref();
  const orderNumber = ref();
  const menuOptions = ref([
    { value: 'RegularFabric(JAKO)', label: 'Regular-Fabric' },
    { value: 'RegularGarment(JAKO)', label: 'Regular-Garment' }
  ])
...
<!--          <Feedback />-->
          ...
          <div style="border: 1px solid #cae2e8;border-bottom: none;padding: 0 10px">
            <label style="font-size: 30px;font-weight: bold">Required Info</label>
          </div>
          <div style="border: 1px solid #cae2e8;border-top: none">
            <RequireLabel
             :buyer="currentBuyer"
