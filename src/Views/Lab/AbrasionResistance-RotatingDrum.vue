  ...
  <div class="allContainer">
    <div class="main">
      ...
      <div class="right-panel">
        <!-- Upper 30% : Report Info (replaced Density Measurement) -->
            ...
            <div style="display:flex; gap:24px; width:100%;">
              ...
              <div style="flex:1; display:flex; flex-direction:column; gap:12px;">
                ...
                <span class="field" style="display:flex; align-items:center;">
                  ...
                  <el-select v-model="cleaningMethodSelected" size="small" style="flex:1; max-width:200px;">
                    <el-option v-for="cm in cleaningOptionsList" :key="cm.value" :label="cm.label" :value="cm.value" />
                  </el-select>
                </span>
              </div>
              ...
              <div style="flex:1; display:flex; flex-direction:column; gap:12px;">
                <span class="field" style="display:flex; align-items:center;">
                  ...
                  <div style="flex:1; max-width:200px; position:relative;">
                    <el-input-number v-model="m1Constant" :precision="4" :step="0.0001" :min="0" size="small" style="width:100%;" controls-position="right" :disabled="loadingConstants" />
                    <el-icon style="position:absolute; right:28px; top:50%; transform:translateY(-50%); cursor:pointer; color:#409eff; z-index:1;" @click="openEditDialog('M1')"><Edit /></el-icon>
                  </div>
                </span>
                ...
                <span class="field" style="display:flex; align-items:center;">
                  ...
                  <div style="flex:1; max-width:200px; position:relative;">
                    <el-input-number v-model="m2Constant" :precision="4" :step="0.0001" :min="0" size="small" style="width:100%;" controls-position="right" :disabled="loadingConstants" />
                    <el-icon style="position:absolute; right:28px; top:50%; transform:translateY(-50%); cursor:pointer; color:#409eff; z-index:1;" @click="openEditDialog('M2')"><Edit /></el-icon>
                  </div>
                </span>
              </div>
                ...
                <table style="width:100%; border-collapse:collapse; margin-top:8px; table-layout:fixed;">
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td style="padding:8px; border-bottom:1px solid #f5f5f5; text-align:center; width:120px; font-weight:600; color:#409eff;">{{ calcTestDensityResult(0) }}</td>
                    </tr>
                    <tr>
                      ...
                      <td style="padding:8px; border-bottom:1px solid #f5f5f5; text-align:center; width:120px; font-weight:600; color:#409eff;">{{ calcTestDensityResult(1) }}</td>
                    </tr>
            ...
            <div style="display:flex; gap:12px; align-items:center; flex-wrap:wrap;">
              ...
              <div v-if="showVolumeLossCalc" style="width:100%;">
                <div style="display:flex; gap:12px; align-items:center; margin-top:6px;">
                  <span class="field">
                    Abrasion distance:
                    <el-select v-model="abrasionDistanceSelected" size="small" style="width:140px; margin-left:8px;">
                ...
                <table style="width:100%; border-collapse:collapse; margin-top:8px; table-layout:fixed;">
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td rowspan="4" style="padding:8px; vertical-align:middle; border-left:1px solid #eee; text-align:center;">ΔV<sub>rel</sub> = (Δm<sub>t</sub> × M₁) / (M₂ × Δm<sub>r</sub>)</td>
            ...
            <div style="display:flex; gap:12px; align-items:center; flex-wrap:wrap;">
              ...
              <div v-if="showARIndexCalc" style="width:100%;">
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td rowspan="4" style="padding:8px; vertical-align:middle; border-left:1px solid #eee; text-align:center; font-size:13px;">
                        I<sub>AR</sub> = (Δm<sub>r</sub> × ρ<sub>t</sub>) / (Δm<sub>t</sub> × ρ<sub>r</sub>) × 100
                      </td>
                ...
                <div style="margin-top:6px; font-size:12px; color:#909399;">
                  <span>ρ<sub>r</sub> (reference rubber density) = 1.15 g/cm³, Δm<sub>r</sub> (reference rubber loss) = 200 mg</span>
                </div>
              </div>
            </div>
          </div>
  ...
  <el-dialog v-model="editDialogVisible" :title="'Modify ' + editForm.type + ' Constant'" width="700px" append-to-body>
    <el-form label-width="110px">
      <el-form-item :label="editForm.type + ' Value'">
        <el-input-number v-model="editForm.value" :precision="4" :step="0.0001" :min="0" style="width:100%;" controls-position="right" />
      </el-form-item>
      <el-form-item label="Modifier Name">
        <el-input v-model="editForm.modifier" placeholder="Enter your name" />
      </el-form-item>
      <el-form-item label="Reason">
        <el-input v-model="editForm.reason" type="textarea" :rows="2" placeholder="Enter the reason for modification" />
      </el-form-item>
      <el-form-item label="Time">
        <el-input :model-value="new Date().toLocaleString()" disabled />
      </el-form-item>
    </el-form>
    <template #footer>
      <el-button @click="editDialogVisible = false">Cancel</el-button>
      <el-button type="primary" @click="submitEditConstant">Submit</el-button>
    </template>
  </el-dialog>
</template>
...
<script setup>
  ...
  import {
    ...
    CirclePlus, RefreshLeft, Delete, Download, Printer, Document
  ...
  const testStandardOptions = [
    { value: 'ASTM D 5963', label: 'ASTM D 5963' },
    { value: 'DIN 53516', label: 'DIN 53516' },
    { value: 'BS 903 A9 Method A', label: 'BS 903 A9 Method A' },
    { value: 'ISO 4649:2024', label: 'ISO 4649:2024' },
    { value: 'BS ISO 4649:2024', label: 'BS ISO 4649:2024' },
    { value: 'EN 12770', label: 'EN 12770' },
    { value: 'ISO 20871', label: 'ISO 20871' },
    { value: 'SATRA TM174', label: 'SATRA TM174' },
    { value: 'QB/T 2884', label: 'QB/T 2884' },
    { value: 'GB/T 9867', label: 'GB/T 9867' },
    { value: 'GB/T 26703', label: 'GB/T 26703' },
  ...
  async function fetchConstants() {
    try {
      ...
      // 假设接口路径为 /api/abrasion/constants，请根据实际情况修改
      const res = await api.get('/api/abrasion/constants')
      if (res.data) {
        m1Constant.value = res.data.m1 ?? null
        m2Constant.value = res.data.m2 ?? null
      }
    } catch (e) {
  ...
  async function submitEditConstant() {
    ...
    try {
      ...
      // 假设接口路径为 /api/abrasion/constants/update，请根据实际情况修改
      await api.post('/api/abrasion/constants/update', payload)
  ...
  // ---- Abrasion Data ----
  const testMode = ref('full') // quarter, half, full
  const referenceRubber = ref('stdA')
  const referenceLoss = ref(200) // mg, reference rubber loss
  ...
  const showARIndexCalc = ref(true)
  ...
  // ---- 修改 onInputFocus：记录焦点位置 ----
  function onInputFocus(areaOrIdx, fieldOrIdx) {
    // 支持多种调用方式：
    // 1. onInputFocus('testDensityM1', 0)  -> area: 'testDensityM1', idx: 0
    // 2. onInputFocus(0, 'before')         -> area: 'abrasion', idx: 0, field: 'before'
    // 3. onInputFocus('refDensityM1', 0)   -> area: 'refDensityM1', idx: 0
    // 4. onInputFocus('densityA')          -> area: 'density', field: 'densityA'
    ...
    // 调试日志（可选，生产环境可移除）
    console.log('Focus set to:', JSON.stringify(activeInput.value))
  }

  // ---- 修改 fillWeightTarget：优先使用焦点位置 ----
  function fillWeightTarget(w) {
    ...
    if (activeInput.value) {
      ...
      console.log('Filling focused input:', JSON.stringify(t))

      // 处理密度表格 (testDensityM1, testDensityM2, refDensityM1, refDensityM2)
      ...
      // 如果焦点区域未匹配，继续尝试顺序填充
      console.log('Focus target not matched, falling back to sequential fill')
    }

    // 2. 无焦点或焦点未匹配：按顺序填充所有可见表格的空输入框
    // 定义填充顺序: 从上到下，从左到右
    // 顺序: testDensityM1[0], testDensityM2[0], testDensityM1[1], testDensityM2[1],
    //       abrasion before[0], abrasion after[0], before[1], after[1], before[2], after[2],
    //       refDensityM1[0], refDensityM2[0], refDensityM1[1], refDensityM2[1]

    // 收集所有需要填充的字段及其值
    const fillTargets = []

    // 2.1 Test Density (始终显示)
  ...
  // Calculate abrasion parameters for a row
  function calculateAbrasion(row) {
    ...
    // ========================================
    // 相对体积损失 (修正为后端公式)
    // ΔV_rel = 1000 × Δm_t × 0.4 / [ρ_t × (M1+M2)] × 里程系数
    // ========================================
    const testDensity = avgTestDensity.value  // 使用测试样品密度平均值
    if (testDensity != null && testDensity > 0) {
      const deltaMt = row.massLoss  // W1-W2 (g)
      const densityInGPerM3 = testDensity  // 换算为 g/cm³
      const distanceFactor = getAbrasionDistanceFactor(abrasionDistanceSelected.value)

      // 400mg = 0.4g
      row.volLoss = 1000 * deltaMt * 400 / (densityInGPerM3 * (m1Constant.value + m2Constant.value)) * distanceFactor
      row.volLoss = Number(row.volLoss.toFixed(4))
    }

    // ========================================
    // 磨耗指数 (修正)
    // I_AR = (Δm_r × ρ_t) / (Δm_t × ρ_r) × 100
    // Δm_r = (M1+M2)/2, ρ_r = 1.15
    // ========================================
    if (testDensity != null && testDensity > 0) {
      const deltaMr = (m1Constant.value + m2Constant.value) / 2  // (M1+M2)/2
      const rhoR = 1.15  // 参考橡胶密度 (g/cm³)
      const deltaMtMg = row.massLoss * 1000  // 转为 mg

      if (deltaMtMg > 0 && rhoR > 0) {
        row.arIndex = (deltaMr * testDensity) / (deltaMtMg * rhoR) * 100
  ...
  function recordAbrasion() {
    // Check all 3 specimens have data
    ...
    // Record to rows for export
    const t = new Date().toISOString()
    abrasionRows.forEach(r => {
      rows.push({
        id: crypto.randomUUID?.() ?? Math.random().toString(36),
        ri: rows.length + 1,
        reportNo: reportNo.value.trim(),      // Report No.
        sample: sampleNumber.value.trim(),    // Sample
        specimen: r.specimen,
  ...
  async function doExport() {
    ...
    try {
      ...
      const map = r => ({
        '#': r.ri,
        'Report No.': r.reportNo,      // 改为 Report No.
        'Sample': r.sample,            // 添加 Sample 字段
        'Specimen': r.specimen,
  ...
  async function generateReport() {
    ...
    try {
      // 构建请求DTO
      const reportData = {
        reportNo: reportNumber.value,  // 使用拼接的完整 Report No
        ...
        testDensities: [
          {
            specimen: 'A',
            m1: testDensityM1.value[0],
            m2: testDensityM2.value[0],
            density: calcTestDensityResult(0) !== '---' ? parseFloat(calcTestDensityResult(0)) : null
          },
          {
            specimen: 'B',
            m1: testDensityM1.value[1],
            m2: testDensityM2.value[1],
            density: calcTestDensityResult(1) !== '---' ? parseFloat(calcTestDensityResult(1)) : null
          }
        ],
  ...
  // ---- Serial functions (same as original, kept for compatibility) ----
...
<style scoped>
  * {
  ...
  /* Field row - label on top, select full width */
  .field-row {
    display: flex;
    flex-direction: column;
    gap: 2px;
  ...
  /* Full width for all form elements */
  .full-width {
    width: 100% !important;
  }
