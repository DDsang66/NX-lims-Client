  ...
  <div class="step3Container">-->
    ...
    <!--<div class="actionButtons">
      ...
      <div class="btnRight">
        ...
        <!-- 修复了原来 type="'info'" 的语法错误，绑定新的打印方法 -->
        <!--<el-button type="info" @click="handleSendToPrinter" :loading="printLoading" :disabled="!docxUrl">
...
const printLoading = ref(false) // 专门给打印按钮用的 loading
const checkListData = ref(null)
const docxUrl = ref('') // 用于存储生成后得到的 Word 文件地址
let offStatus = null
...
// 1. Generate 按钮：仅负责生成并保存 docx 文件地址
async function handleGenerate() {
  ...
  try {
    ...
    if (res.data.isSuccess) {
      ...
      // ✅ 拼接成完整 URL 并存储为 Word 地址
      docxUrl.value = rawUrl.startsWith('http')
        ? rawUrl
        : `${BACKEND_BASE}${rawUrl}`

      console.log("Generated DOCX url:", docxUrl.value)
    } else {
...
// 2. Send to Printer 按钮：负责将已生成的 Word 文档发送给打印服务
  async function handleSendToPrinter() {
    ...
    try {
      // 使用 printWord 替代 printPdf
      await printWord(docxUrl.value, {
        copies: 1,
